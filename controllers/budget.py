...
async def get_all_budgets_of_family(family_id: str, current_user: UserModel, db: AsyncSession)->RestGetAllBudgetsOfamilyResponse:
    ...
    return RestGetAllBudgetsOfamilyResponse(code=1, status="SUCCESS", message="Family budgets retrieved successfully", budgets=[BudgetInfo(**budget) for budget in family.budgets])
...
async def create_budget_for_family(family_id: str, new_budget: CreateBudget, current_user: UserModel, db: AsyncSession)-> RestCreateBudgetResponse:
    ...
    if not family:
        return BaseRestResponse(code=0, status="FAILED", message="Family not found")
    # Create new budget
    new_budget = BudgetModel(**new_budget.model_dump(), family_id=UUID(family.id), user_id=current_user.id)
    db.add(new_budget)
    try:
        await db.commit()
        await db.refresh(new_budget)
        return RestCreateBudgetResponse(code=1, status="SUCCESS", message="Budget created successfully", budget=BudgetInfo(**new_budget.model_dump()))
    except:
        await db.rollback()
        return BaseRestResponse(code=0, status="FAILED", message="Failed to create budget")

async def retrieve_budget(budget_id: str, current_user: UserModel, db: AsyncSession)->RestGetBudgetResponse:
    # Check if the user is a member of the family
    await check_user_in_family(budget_id, current_user.id, db)
    # Get budget
    budget = await get_budget_by_id(budget_id, db)
    if not budget:
        return BaseRestResponse(code=0, status="FAILED", message="Budget not found")
    return RestGetBudgetResponse(code=1, status="SUCCESS", message="Budget retrieved successfully", budget=BudgetInfo(**budget.model_dump()))
...
async def update_budget(budget_id: str, updated_budget: UpdateBudget, current_user: UserModel, db: AsyncSession)->RestCreateBudgetResponse:
    # Check if the user is the owner of the family
    await check_user_is_family_owner(budget_id, current_user.id, db)
    # Get budget
    budget = await get_budget_by_id(budget_id, db)
    if not budget:
        return BaseRestResponse(code=0, status="FAILED", message="Budget not found")
    ...
    try:
        ...
        return RestCreateBudgetResponse(code=1, status="SUCCESS", message="Budget updated successfully", budget=BudgetInfo(**budget.model_dump()))
    except:
        await db.rollback()
        return BaseRestResponse(code=0, status="FAILED", message="Failed to update budget")
